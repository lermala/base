...
```bash
git diff HEAD путь/к/файлу # сравнение с последним коммитом
git diff ваш_коммит_хеш путь/к/файлу # сравнение с конкретным коммитом
```

### Сравнение с проиндексированными изменениями

```bash
git diff --cached путь/к/файлу
git diff --staged путь/к/файлу
```

## Теги
Git tag — это команда для работы с тегами в системе контроля версий Git. Теги используются для обозначения важных моментов в истории проекта, например, выпусков версий (v1.0, v2.0), значимых этапов разработки или стабильных состояний кода.

Подробнее [Основы Git - Работа с тегами](https://git-scm.com/book/ru/v2/Основы-Git-Работа-с-тегами)

### Создание
В Git есть два основных типа тегов:
1. Легковесные (lightweight) — простые указатели на коммит, не содержащие дополнительной информации (автора, даты, сообщения). Это быстрый способ создать ссылку на коммит. 
```bash
git tag <tagname>
```
Например, git tag v1.0 создаст легковесный тег с именем v1.0, указывающий на текущий коммит. 
2. Аннотированные (annotated) — полноценные объекты в базе данных Git, которые содержат дополнительные метаданные: имя автора, email, дату создания, сообщение и возможность подписи с помощью GPG. Рекомендуется для официальных релизов. 

```bash
git tag -a <tagname> -m "message"
```
Опция -m задаёт сообщение, которое будет храниться вместе с тегом. Если сообщение не указано, Git запустит редактор для его ввода.
