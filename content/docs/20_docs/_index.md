...
## Предпроектный этап
### Документ о концепциях и границах, BRD
...
Тут подходят Сценарии использования. Далее на их основании формируются функциональные требования уже в SRS/ТЗ. Это не всегда отдельный документ, это может быть частью SRS/ТЗ.

### Каталог бизнес-правил

![alt text](image-1.png "Пример каталога бизнес-правил")
...
**ТЗ** – техническое задание. Можно сказать, что это аналог SRS, но на русские стандарты ГОСТ.
...
***Проектное решение / Технический проект** — это детальный план, который описывает, как именно будет решаться конкретная задача или создаваться система в рамках проекта.

**Технорабочий проект, ТРП** — это комплексная документация, которая объединяет в себе две стадии проектирования: технический проект и рабочую документацию. Такой гибридный формат позволяет сразу переходить от планирования к реализации без промежуточных стадий согласования.

## Тестирование
### ПМИ / ПиМИ
**Программа и методика испытаний (ПМИ/ПиМИ)** — это технический документ, который детально регламентирует процесс проверки какого-либо объекта: оборудования, автоматизированной системы, программного продукта или материала.

В этом документе последовательно описываются:

- Объект испытаний — его наименование, версия, область применения и ключевые характеристики.
- Цель и вид испытаний — что именно подтверждается (например, соответствие техническим требованиям, безопасность).
- Условия проведения — режимы, окружающая среда (температура, влажность), используемое оборудование и стенды.
- Средства испытаний — приборы, стенды, тестовые данные, программные средства.
- Порядок и методы проверок — последовательность действий, способы измерений и контроля.
- Критерии оценки — при каких результатах проверка считается пройденной, предельные значения параметров.
- Порядок оформления и обработки результатов — формы протоколов, таблиц, актов, итоговое заключение.

### Протокол испытаний

**Протокол испытаний** — это официальный документ, который составляют после проверки продукции, оборудования или материалов. В нём подробно фиксируют всё, что происходило во время исследований: какие методы и оборудование использовали, при каких условиях проводили тесты и, главное, какие получились результаты.

ПМИ составляется до начала испытаний и задаёт их план, а протокол испытаний фиксирует фактические результаты после их завершения.
...
|  № | ГОСТ 34: стадия создания АС      | Что происходит по ГОСТ 34                                                                                              | Ближайшая стадия ГОСТ 19                      | Документация ГОСТ 34                                                                                                                                  | Документация ГОСТ 19 / ЕСПД                                                                       | Как соотносятся                                                                                                  |
| -: | -------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
|  1 | **Формирование требований к АС** | Обследование объекта, обоснование необходимости АС, формирование требований пользователя                               | Прямого аналога нет                           | Отчёт об обследовании/выполненной работе; заявка на разработку АС (ТТЗ)                                                                               | —                                                                                                 | ГОСТ 34 начинается **раньше**, чем жизненный цикл разработки конкретной программы по ГОСТ 19                     |
|  2 | **Разработка концепции АС**      | Изучение объекта, НИР, варианты концепции, выбор решения                                                               | Прямого аналога нет; частично предшествует ТЗ | Отчёт о НИР; описание и обоснование концепции, ТЭО и др.                                                                                              | При необходимости материалы ТЭО используются при подготовке ТЗ                                    | Это системный уровень: решается, **что за АС создавать и каким образом**                                         |
|  3 | **Техническое задание**          | Разработка, согласование и утверждение ТЗ на АС                                                                        | **Техническое задание**                       | **ТЗ на создание АС** по ГОСТ 34.602                                                                                                                  | **ТЗ на программу** по ЕСПД                                                                       | Названия стадий совпадают, но объекты разные: в ГОСТ 34 — **АС**, в ГОСТ 19 — **программа/программный комплекс** |
|  4 | **Эскизный проект (ЭП)**         | Предварительные проектные решения по системе и её частям                                                               | **Эскизный проект**                           | Ведомость ЭП, пояснительная записка, схемы, описания предварительных решений и др.                                                                    | Пояснительная записка и иные предусмотренные ТЗ программные документы                             | Хорошее соответствие по смыслу: предварительное проектирование                                                   |
|  5 | **Технический проект (ТП)**      | Детальные проектные решения по АС: архитектура, функции, данные, ПО, технические средства, организационное обеспечение | **Технический проект**                        | Ведомость ТП; пояснительная записка; описание автоматизируемых функций; описание информационного, программного, технического обеспечения; схемы и др. | Пояснительная записка и другие предусмотренные ТЗ документы                                       | Также близкое соответствие, но ГОСТ 34 описывает **всю систему**, а ГОСТ 19 — программную реализацию             |
|  6 | **Рабочая документация**         | Разработка рабочей документации; разработка или адаптация программ                                                     | **Рабочий проект**                            | Рабочая и эксплуатационная документация АС; инструкции; описания; технологические документы и др.                                                     | Спецификация; текст программы; описание программы; ПМИ; эксплуатационные документы и др.          | **Главная точка стыковки ГОСТ 34 и ГОСТ 19**: программная часть АС документируется по ЕСПД                       |
|  7 | **Ввод в действие**              | Подготовка объекта и персонала, ПНР, предварительные испытания, опытная эксплуатация, приёмочные испытания             | Частично **Рабочий проект / внедрение**       | ПМИ АС; протоколы испытаний; акты приёмки в опытную эксплуатацию, завершения ОЭ и приёмки в постоянную эксплуатацию; эксплуатационная документация    | ПМИ программы; эксплуатационные документы; при необходимости корректировка программных документов | ГОСТ 34 значительно подробнее регламентирует **ввод системы в эксплуатацию**                                     |
|  8 | **Сопровождение АС**             | Гарантийное и послегарантийное обслуживание, изменение АС и документации                                               | Прямой стадии в ГОСТ 19.102 нет               | Изменения системной и эксплуатационной документации                                                                                                   | Изменения программной и эксплуатационной документации ЕСПД                                        | ГОСТ 34 охватывает эксплуатационную жизнь системы шире                                                           |
...
- [Скачать шаблон ТЗ/ЧТЗ по ГОСТ_34.602](/doc-templates/tz_gost34_template.docx)
